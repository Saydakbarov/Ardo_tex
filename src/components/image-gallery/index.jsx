      ...
      <div className="w-full h-full bg-white md:rounded-md  relative p-8 ">
        ...
        {data?.map((item, index) => (
          ...
          >
           ...
           {
            type == "material" && (
              <span className="text-lg md:text-2xl font-bold absolute z-[2] text-white top-[20px]  md:top-[50px]  md:right-[50px]  right-[20px] w-fit block p-[4px] rounded-md bg-[#000000b7] ">{numbers ?  numbers[index] : "0"}</span>
            )
           }
          </div>
        ))}
